...
Статус: частично реализовано в `messenger.html` как `window.MacaroniGitHub` и описано в `docs/github-provider.md`. Первый provider - GitHub через REST Contents API. Adapter умеет parse repo URL, read file/json, list directory, write file/json с Base64 content и `sha` при update. Если в профиле есть GitHub token, composer пишет через GitHub Contents API; без token GitHub repo работает как read-only public repo. GitHub Pages demo автоматически создаёт read-only demo profile и индексирует захардкоженный `.macaroni` dataset внутри HTML, чтобы новый пользователь сразу увидел чаты без token и без unauthenticated GitHub API calls; тот же режим можно включить явно через `?demo=1`, а `?setup=1` возвращает обычный onboarding. UI показывает текущий transport, sync state и outbox count. Sync пока простой: все chat meta, обход messages по `YYYY/MM/DD`, плюс чтение `.macaroni/inbox/<CLIENT_ID>` как receive hint, без Git Trees API. Перед полным GitHub reindex клиент проверяет последний commit SHA branch и пропускает обход Contents API, если repo не изменился. Для GitHub-профиля включён polling: 30 секунд с token, 60 секунд read-only; клиент не ставит hidden tabs на паузу, но браузер может throttling-ить фоновые таймеры. Generic git provider contract зафиксирован в `docs/generic-git-provider.md`: `.macaroni/` layout может жить в любом git repo, но browser tab требует host-specific HTTPS/CORS adapter или wrapper, а не магический raw SSH git.
...
Reindex/reset controls находятся в настройках. `Сбросить` после confirm удаляет профиль, token, индекс и local test repo; `Пересобрать индекс` пересобирает только локальный кеш из выбранного provider/repo и не создаёт новые remote-файлы.

Destructive reset подтверждается встроенным dialog, а не browser `confirm`, чтобы основной UI не разваливался на нативные модалки.
...
Composer draft хранится отдельно для каждого `chat_id` в памяти текущей вкладки. При переключении чатов поле ввода восстанавливает draft выбранного чата и не переносит текст из другого чата; git draft'ы не хранит.
...
Переключение чатов должно быть быстрым: заголовок, active state и сообщения выбранного чата обновляются сразу, а read receipt, unread counters и полный sidebar rerender выполняются после и не блокируют клик.

Кнопка `+` открывает встроенную панель создания чата, а не browser prompt. После submit клиент создаёт чат, добавляет текущий `CLIENT_ID` и default peer в `members.json`, затем сразу открывает созданный чат.

`Инфо о чате` открывает встроенную панель, а не `alert`: title, `chat_id`, участников из `members.json`, статус текущего `CLIENT_ID`, transport и размер outbox. Если текущего пользователя нет в `members.json`, панель показывает явную кнопку `Вступить в чат`.
...
1. Git-agnostic provider adapters: реализован minimal runtime registry `window.MacaroniRemoteAdapters` для `GitHub`, `GitLab`, `GitVerse`, `Gitea`, `Forgejo`. `Generic Git HTTP` остаётся contract-only и показывает честную ошибку вместо silent fallback.
   ...
   - Storage branch и read-only composer guard реализованы в 1.04: `.macaroni/` может жить в отдельной ветке, а read-only profile больше не показывает composer.
...
3. Read-only public repo mode: сделано для remote profiles без token; клиент читает историю, показывает provider-specific read-only transport, не создаёт чаты, не отправляет сообщения и скрывает composer без write token.
4. URL attachments: частично сделано как безопасный auto-link `http://`/`https://` в тексте сообщения, без бинарных файлов и preview.
5. Markdown rendering: сделан безопасный inline-render `**bold**`, `*italic*`, `` `code` `` в UI сообщений, без HTML passthrough. Полноценный CommonMark-движок не делаем.
6. Basic notifications: сделано в текущем scope как unread count в `document.title` и встроенный sound для новых входящих сообщений, без Browser Notification API и permission prompts.
