...
## Математика Для Нетерпеливых

У нас всё тупо, примитивно и в лоб.

Это правда.

Но размер secret/salt очень сильно меняет разговор.
...
то вы сами принесли деревянную дверь и попросили считать её сейфом.

Если вы используете не связанные между собой PGP-sized secrets как `secret` и `salt`, разговор становится другим.

Даже если от всей этой роскоши практически полезными окажутся "всего" 128 или 256 бит энтропии, перебор выглядит так:

```text
2^128 вариантов ~= 3.4e38
2^256 вариантов ~= 1.1e77
```

Даже при фантастических `10^12` проверок в секунду `2^128` - это примерно `10^19` лет.

Возраст Вселенной - примерно `10^10` лет.

То есть если вы зашифровали:

```text
мам, свари макароны
```

с двумя жирными, не связанными друг с другом PGP-sized кусками key material, мы бы хотели дожить до момента, когда кто-то расшифрует это честным перебором.

Скорее всего, не доживём.

Сильный secret переносит атаку из "подобрать ключ" в "добыть ключ".

То есть злоумышленник скорее пойдёт:

- воровать `messenger.html`;
- вытаскивать `localStorage`;
- искать backup;
- подменять hosted HTML;
- ловить clipboard;
- заниматься старой доброй офлайн-криптографией с паяльником.

Это не доказывает, что наш велосипед стал PGP.

Это доказывает, что жирный secret превращает перебор в плохой план.
